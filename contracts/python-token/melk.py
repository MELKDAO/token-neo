...
def get_role_owner(role: bytes) -> UInt160:
    return UInt160(storage.get(role))


def set_role_owner(role: bytes, account: UInt160) -> None:
    assert runtime.check_witness(get_role_owner(role))
    storage.put(role, account)
